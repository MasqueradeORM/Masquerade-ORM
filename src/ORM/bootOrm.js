...
import { floatColumnTypes } from "../misc/constants.js"
...
export function nodeArr2ClassDict(nodeArr) {
    /**@type {any}*/ let classArr = filterNodesAndConvert2ClassObjects(nodeArr)
   const entityArr = classArr.map(classObj => classObj.name)
   for (const classObj of classArr) {
...
export function filterNodesAndConvert2ClassObjects(nodeArr) {
   ...
   while (currentValidParents.length) {
      for (const node of currentUnfilteredNodes) {
         const parent = node.heritageClauses[0].types[0].expression.escapedText
...
export function inheritColumns(inheritingClass) {
   ...
   while (iterativeClassObj.parent) {
      ...
      if (iterativeClassObj.specialSettings
         && iterativeClassObj.specialSettings.idType
         && iterativeClassObj.parent.name === 'Entity'
      ) {
         const index = inheritingClass.columns.findIndex(columnObj => columnObj.name === 'id')
...
function addRefersOrDeps(added, to) {
   for (const [dependentClass, dependentProps] of Object.entries(added)) {
      const target = to[dependentClass] ??= []
      target.push(...dependentProps)
   }
}
...
export function linkClassWikis(classWikiDict, dependenciesObj, referencesObj) {
   for (const classWiki of Object.values(classWikiDict)) {
      ...
      if (dependenciesObj[className]) classWiki.$dependents = dependenciesObj[className]
      if (referencesObj[className]) classWiki.$referencers = referencesObj[className]

      for (const [propertyName, junction] of Object.entries(junctions)) {
         classWiki[propertyName] = classWikiDict[junction.$className]
      }
   }

   for (const classWiki of Object.values(classWikiDict)) {
      const parentName = classWiki.$parent
      if (parentName === 'Entity') {
         delete classWiki.$parent
         continue
      }
      let currentParent = classWikiDict[parentName]
      while (currentParent) {
         const { $className, $tableName, $parent, $junctions, $relationalProps, ...rest } = currentParent
         classWiki.$relationalProps.push(...$relationalProps)
         for (const [prop, val] of Object.entries(rest)) classWiki[prop] = val
         const dependends = currentParent.$dependents
         const referencers = currentParent.$referencers
         if (dependends) {
            const classWikiDependents = classWiki.$dependents ??= {}
            addRefersOrDeps(dependends, classWikiDependents)
         }
         if (referencers) {
            const classWikiReferencers = classWiki.$referencers ??= {}
            addRefersOrDeps(dependends, classWikiReferencers)
         }
         if (!$parent) break
         currentParent = classWikiDict[$parent]
      }
      delete classWiki.$parent
   }

   for (const classWiki of Object.values(classWikiDict)) {
      for (const [propertyName, { $isArray: isArray, $optional: optional, $junctionInfo }] of Object.entries(classWiki.$junctions)) {
         const target = classWiki[propertyName] = { ...classWiki[propertyName], $junctionInfo }
         delete target.$junctions
         if (isArray) target.$isArray = true
         if (optional) target.$optional = true
      }
...
export function createClassWiki(tableObj) {
   ...
   for (const tableName of tableNames) {
      ...
      for (const columnObj of Object.values(table.columns)) {
         ...
         if (columnObj.relational) {
            ...
            if (!$isArray && !$optional) {
               ...
               const dependantKeyArr = targetTable[tableName] ??= []
               dependantKeyArr.push(propName)
            }
            else {
         ...
         if ($optional) target.$optional = $optional
      }
        /**@type {any}*/ const map = {
         $className: tableName,
      ...
      if (table.parent) map.$parent = table.parent.name
...
export function createTableDict(branchArr) {
   let tableDict = {}
   for (const branch of branchArr) {
      if (branch.parent) {
         const newBranch = inheritColumns(branch)
         if (newBranch) branchArr.push(newBranch)
         tableDict[branch.name] = branch
      }
   }
   for (const tableObj of Object.values(tableDict)) {
      const { parent, children, specialSettings } = tableObj
      if (specialSettings && specialSettings.idType && parent.name === 'Entity' && children) {
         for (const childName of children) inheritIdType(childName, tableObj.columns.find(c => c.name === 'id'), tableDict)
      }
   }
   return tableDict
}

function inheritIdType(childName, idType, tableDict) {
   const child = tableDict[childName]
   child.columns.find(c => c.name === 'id').type = idType.type
   if (child.children) {
      for (const childName of child.children) inheritIdType(childName, idType, tableDict)
   }
}
