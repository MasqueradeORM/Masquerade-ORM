...
export class Entity {
  ...
  /**@type {Date}*/ updatedAt
   ...
   constructor() {
      ...
      const { classWikiDict, idLogger, entityMapsObj, mutationsLog } = OrmStore.store
      ...
      const classChangeObj = mutationsLog[className] ??= {}
   ...
   */
   static async find(findObj) {
      const { dbConnection, sqlClient, entityFunctions, classWikiDict } = OrmStore.store
      ...
      const instanceArr = sqlClient === "postgres"
         ? postgresCreateProxyArray(queryResult, relationsScopeObj, entityFunctions, relationsArg)
         : sqliteCreateProxyArray(queryResult, relationsScopeObj, entityFunctions, relationsArg, !!statementsObj.orderByStr)
   ...
   */
   static async fetch(id) {
      ...
      const { sqlClient, dbConnection, entityFunctions, classWikiDict } = OrmStore.store
      if (!classWikiDict) throw new Error("ORM is not initialized. Please call the appropriate ORM boot method before use.")
      ...
      try {
         ...
         const instanceClass = entityFunctions[className]
   ...
   */
   delete() {
      const { dbConnection, classWikiDict, dependentsMapsObj, mutationsLog } = OrmStore.store
      if (!dbConnection) throw new Error("ORM is not initialized. Please call the appropriate ORM boot method before use.")
      ...
      const deletionDict = mutationsLog.$deletedInstances ??= {}
      const classDeletionArr = deletionDict[nonSnake2Snake(targetTableName)] ??= []
      classDeletionArr.push(id4Deletion)
      if (mutationsLog[className] && mutationsLog[className][id4Deletion]) delete mutationsLog[className][id4Deletion]
   ...
   */
   async fetchDbState() {
      const { dbConnection, classWikiDict, sqlClient, entityFunctions, mutationsLog } = OrmStore.store
      if (!dbConnection) throw new Error("ORM is not initialized. Please call the appropriate ORM boot method before use.")
      const className = this.constructor.name
      const classWiki = classWikiDict[className]
      if (!classWiki) throw new Error(`The class ${className} is not integrated into the ORM. Please include it in the boot method and restart.`)
      ...
      for (const [relationalProp, joinedTableWiki] of Object.entries(junctions)) {
         ...
         queryResult.forEach(row => {
            ...
            return rowObj2InstanceProxy(rowWithCamelCasedProps, joinedTableWiki, entityFunctions)
         })
      ...
      delete mutationsLog[className][this.id]
   }
}
